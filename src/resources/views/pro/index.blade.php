...
@section('css')
<link rel="stylesheet" href="{{ asset('css/pro/index.css') }}">
...
<div class="restaurant__form">
    <!-- ソートフォーム -->
    <form class="restaurant__sort-form" action="/pro/sort" method="get">
        @csrf
        <div class="sort-form__item">
            ...
            <select class="sort__form-select" name="sort_id" onChange="this.form.submit()">
                <option hidden>並び替え：評価高/低</option>
                <option hidden @if( $sort_id==1 ) selected @endif>ランダム</option>
                <option value="1">ランダム</option>
                <option value="2" @if( $sort_id==2 ) selected @endif>評価が高い順</option>
                <option value="3" @if( $sort_id==3 ) selected @endif>評価が低い順</option>
            </select>
        </div>
    </form>
    <!-- 検索フォーム -->
    <form class="restaurant__search-form" action="/pro/search" method="get">
        @csrf
